  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${redirectTo}?redirect=${encodeURIComponent("/app")}${
            priceId ? `&priceId=${encodeURIComponent(priceId)}` : ""
          }${
            discountCode ? `&discountCode=${encodeURIComponent(discountCode)}` : ""
          }`,
        },
      });
            ...
            <div className="space-y-6">
              <form action={magicLinkAction} className="space-y-4">
                <Input
                  ...
                  required
                  className="px-4 h-12 bg-white rounded-lg text-black border-gray-200 shadow-sm transition-colors focus:border-blue-500 focus:ring-blue-500"
                />
